...
class Generator 
{
    ...
    public function validateField($form_service_name, $field_to_compare)
    {
        ...
        foreach($field->getErrors() as $idx => $error){
                $errors[] =  $this->container->get('translator')->trans($error->getMessage(), array(), $config['validations'][$form_service_name]['translation_domain'],  $this->container->get('request')->getLocale());
        }
